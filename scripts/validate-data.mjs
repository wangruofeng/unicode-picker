...
if (manifest.symbolCount !== EXPECTED_COUNT || catalog.length !== EXPECTED_COUNT) {
  throw new Error(`Expected ${EXPECTED_COUNT} records, got manifest=${manifest.symbolCount}, catalog=${catalog.length}`);
}
...
for (const symbol of catalog) {
  ...
  if (!allowedCategories.has(symbol.generalCategory)) throw new Error(`Invalid category for ${symbol.id}: ${symbol.generalCategory}`);
